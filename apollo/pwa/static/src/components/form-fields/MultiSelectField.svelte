...
<div class="mb-3">
    ...
    {#each options as [label, option_value], index}
        <div class="form-check">
            <label for={ `${field.tag}_${index}` } class="form-label">{ label }</label>
            <input type="checkbox" name={ field.tag } id={ `${field.tag}_${index}` } bind:group={ value } class="form-check-input" value={ option_value } disabled={ locked }>
        </div>
    {/each}
</div>